...
- トグルの意味は再生制御に直結させる。
  - プロジェクトOFF: そのプロジェクト配下の再生は開始不可。実行中なら即時停止。
  - 設定ファイルOFF: 当該設定ファイルの再生は開始不可。実行中なら即時停止。
...
- 設定ファイル詳細からのスケジュール開始が可能
- 画面遷移後もタイマー再生を継続できる
- プロジェクト/設定ファイルのトグル OFF で再生停止できる
- トグル ON に戻した後に再開できる
- バックグラウンド状態でも再生できる
- Youtube Music 再生中に音声を重ねて再生できる
- Audio Focus の ducking リクエストを追加し、Youtube Music 側の一時減衰を確認

### 直近で対応したポイント

- NativePlayback プラグインの登録順を見直し（MainActivity 起動時の登録）
- 保存済み targetPath 再生時のキュー復元を調整
- 外部音声ファイル再生での権限不足（EACCES）に対して、READ_MEDIA_AUDIO / READ_EXTERNAL_STORAGE の実行時要求を追加
- NativePlaybackService で再生失敗理由を詳細に返すよう改善
- MediaPlayer の stop 状態エラーログを抑制

### 既知の注意点

- Ducking の最終的な効き方は再生元アプリの実装依存
- Download 配下など外部ファイル再生では Android 権限が必要
- 端末によっては権限再許可や再インストール後に挙動が安定する場合がある